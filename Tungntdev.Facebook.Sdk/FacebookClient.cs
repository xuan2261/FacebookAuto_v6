...
namespace Tungntdev.Facebook.Sdk
{
    public class FacebookClient : IDisposable
    {
        ...
        public FacebookClient(ILogger log = null)
        {
        ...
        //-------------------Detail_AccountFb-----------------//
        public tblAccountFB GetDetailAccount(string user, string pass)
        {
            ...
            // create content payload
            string credential = string.Format("email={0}&pass={1}", user, pass);
            string payload = HtmlHelper.BuildPayload(inputs, additionKeyValuePair: credential);
            ...
            using (HttpWebResponse response = _http.SendPostRequest("https://mobile.facebook.com/login.php", payload))
            {
                ...
                else
                {
                    ac.Email = user;
                    ac.Password = pass;
                    ac.NumberIDAccount = response.Cookies["c_user"].Value.ToString();
                    document = this.BuildDom("https://mobile.facebook.com/profile.php?v=info");
                    try
                    {
                        ac.Name = document.SelectSingleNode("/html/body/div/div/div[2]/div/div[1]/div[1]/div[2]/div/div[1]/span/div/span/strong").InnerText;
                    }
                    catch { }
                    try
                    {
                        ac.Phone = document.SelectSingleNode("/html/body/div/div/div[2]/div/div[1]/div[6]/div/div[2]/div[1]/table/tr/td[2]/div/span/span").InnerText;
                    }
                    catch { }
                    try
                    {
                        ac.Sex = document.SelectSingleNode("/html/body/div/div/div[2]/div/div[1]/div[7]/div/div[2]/div[2]/table/tr/td[2]/div").InnerText;
                    }
                    catch { }
                    try
                    {
                        ac.Address = document.SelectSingleNode("/html/body/div/div/div[2]/div/div[1]/div[5]/div/div[2]/div[1]/div/table/tr/td[2]/div/a").InnerText;
                    }
                    catch { }
                    try
                    {
                        ac.AddressNow = document.SelectSingleNode("/html/body/div/div/div[2]/div/div[1]/div[5]/div/div[2]/div[2]/div/table/tr/td[2]/div/a").InnerText;
                    }
                    catch { }
                    try
                    {
                        ac.Birthday = document.SelectSingleNode("/html/body/div/div/div[2]/div/div[1]/div[7]/div/div[2]/div[1]/table/tr/td[2]/div").InnerText;
                    }
                    catch { }
                    try
                    {
                        ac.ImageLink = document.SelectSingleNode("/html/body/div/div/div[2]/div/div[1]/div[1]/div[2]/div/div[1]/div/div/a/img").InnerText;
                    }
                    catch { }
                }
            }
        ...
        //---------------GetDetailPost--------------//
        public tblPost GetDetailPost(string idpost)
        {
            tblPost p = new tblPost();
            HtmlNode document = this.BuildDom("https://mobile.facebook.com/"+idpost);
            try
            {
                var t = document.SelectSingleNode("/html/body/div/div/div[2]/div/div[2]/div/div/div[1]/div[2]/div/div/div/span");
                if (t == null)
                {
                    t= document.SelectSingleNode("/html/body/div/div/div[2]/div/div[2]/div/div/div[1]/div[2]/p");
                }
                p.Description = t.InnerText;
            }
            catch {
        ...
        //----------------UpBinhLuanMoi-----------------//
        public void UpBinhLuanMoi(string idpost, string noidungcomment, string idaccountbl)
        {
            ...
            // create content payload
            string credential = string.Format("email={0}&pass={1}", dt.Rows[0]["Email"], dt.Rows[0]["Password"]);
            string payload = HtmlHelper.BuildPayload(inputs, additionKeyValuePair: credential);
            ...
            document = this.BuildDom("https://mobile.facebook.com/mbasic/comment/advanced/?target_id="+idpost+"&pap&at=compose");
            HtmlNode postForm = document.SelectSingleNode("//form[contains(@action, 'https://upload')]");
            inputs = postForm.ParentNode.Elements("input");
            inputs.ElementAt(1);
            payload = HtmlHelper.BuildPayload(inputs, "photo=&post=Post&comment_text=" + Uri.EscapeDataString(noidungcomment));
            string actionUrl = postForm.GetAttributeValue("action", null);
            actionUrl = actionUrl.Replace("amp;", "");
            using (HttpWebResponse response = _http.SendPostRequest(actionUrl, payload))
            {
                
            }
        }
        //----------------KetThucUpBinhLuanMoi-----------------//

        //----------------ChiaSeBaiViet-----------------//
        public void ChiaSeBaiViet(string idpost, string noidungchiase,string idtaikhoancs)
        {
            DataTable dt = AccountFB.LoadDuLieuByID(idtaikhoancs);
            HtmlNode document = this.BuildDom("https://mobile.facebook.com");
            // Get login form Dom object
            HtmlNode loginForm = document.SelectSingleNode("//form[@id='login_form']");
            IEnumerable<HtmlNode> inputs = loginForm.ParentNode.Elements("input");
            ...
            //chia sẻ
            document = this.BuildDom("https://mobile.facebook.com/composer/mbasic/?c_src=share&referrer=permalink&target="+idtaikhoancs+"&sid="+idpost);
            HtmlNode postForm = document.SelectSingleNode("//form[contains(@action, '/composer')]");
            inputs = postForm.ParentNode.Elements("input");
            inputs.ElementAt(1);
            payload = HtmlHelper.BuildPayload(inputs, "view_post=Share&shared_from_post_id="+idpost+ "&xc_message=" + Uri.EscapeDataString(noidungchiase));
            string actionUrl = postForm.GetAttributeValue("action", null);
            actionUrl = actionUrl.Replace("amp;", "");
            using (HttpWebResponse response = _http.SendPostRequest("https://mobile.facebook.com" + actionUrl, payload))
            {
